...
- `src/lib/api.ts` – fetch client for the backend: `getBoard` (404 -> null), `saveBoard`
  (POST), `updateCard` (PATCH), `removeCard` (DELETE). Base URL:
...
- `src/lib/BoardContext.tsx` – `BoardProvider`/`useBoard`: loads the board for the
  ...
  and exposes `renameColumn` / `addCard` / `moveCard` / `deleteCard`. Actions update the
  UI optimistically and then persist (moves via the PATCH endpoint, the rest via a full
  board POST).
